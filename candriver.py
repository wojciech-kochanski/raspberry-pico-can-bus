...
class CanSignal():
    ...
    def __init__(self, start_byte, start_bit, length, scalar=1, offset=0, unit='', name='', value=0):
    ...
    def SetPhysicalValue(self, value):
        self.value = int((value - self.offset) // self.scalar)
        # print('Setting ph. value, signal %s, value = %d, offset = %d, scalar = %d, calculated raw value = %d = 0x%X' % (self.name, value, self.offset, self.scalar, self.value, self.value)) 
    ...
    def __str__(self):
        return "%s: %d %s" % (self.name, self.value * self.scalar + self.offset, self.unit)
...
class CanFrame():
    ...
    def SetPayloadFromSignals(self):
        ...
        for signal in self.signals:
            ...
            # print("calculating payload for signal %s, signal value = %X, mask = %X, shift = %d" % (signal.name, signal_value, mask, shift))
            
            #clear the payload area of signal - delete previous values            
            payload &= self.GetInvertedMask(mask)
            
            # put new values into the payload
    ...
    # returns inverted mask of width = dlc, for use with signals
    def GetSignalMaskAndShift(self, signal):
        shift = (self.dlc*8 - signal.start_byte*8 - signal.start_bit - signal.length)
        mask = ((1 << signal.length) -1) << shift
        return mask, shift
        
        
    def GetInvertedMask(self, mask):
        full_mask = (1<<self.dlc*8) -1        
...
if __name__ == '__main__':
    ...
    '''
    frame_ext = CanFrame(id=0x512, payload =[1, 2, 3, 4, 5, 6, 7, 8], extended=True)    
    can.Send(frame_ext)
    time.sleep(1)
